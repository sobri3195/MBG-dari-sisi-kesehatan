...
export default function EntryCheckForm() {
  ...
  const handleScanQR = async () => {
    ...
    setLoading(true);
    try {
      const clearanceData = await api.healthClearance.validateQR(qrInput);
      if (!clearanceData) {
        alert('QR Code tidak valid atau clearance tidak ditemukan/kadaluwarsa');
        return;
      }
      
      const personnel = await api.personnel.getById(clearanceData.personnel_id);
      setClearance({
        ...clearanceData,
        personnel,
      });
